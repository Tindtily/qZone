(window.constructQZFL = function () {
    ...
    (function (w) {
        ...
        do try {
            ...
            cw.imgcacheDomain &&
            (QZFL.config.domain = cw.imgcacheDomain.replace("http://", "").split("/")[0])
        } catch (err) {
            break
        }
    ...
    (typeof QZFL.config.domainPrefix ==
    "undefined") && (QZFL.config.domainPrefix = "qq.com");
    ...
    QZFL.css = {
        ...
        rgb2hsl: function (r, g, b) {
            var t, red = Math.max(r / 255, 0),
                ...
                result = {h: 0, s: 0, l: Math.max((max + min) / 2, 0)};
            if (max != min) {
                if (max == red) result.h = (t = 60 * ((green - blue) / (max - min))) < 0 ? t + 360 : t;
        ...
        getRuleBySelector: function (sheetId, selector) {
            ...
            for (var i = 0, len = _rs.length; i < len; ++i)
                if (selector == _rs[i].selectorText.toLowerCase()) return _rs[i];
            return null
        },
        ...
        hasClassName: function (elem, name) {
            return elem && name ? elem.classList ?
                elem.classList.contains(name) : name && (" " + elem.className + " ").indexOf(" " + name + " ") > -1 : false
        },
            ...
            if (elems && typeof elems == "object") {
                ...
                for (var elem, i = 0, l = elems.length; i < l; ++i)
                    if ((elem = elems[i]) && elem.nodeType == 1)
                        if (QZFL.css.hasClassName(elem, a)) QZFL.css.updateClassName(elem, a, b);
                        else if (!_isRep && QZFL.css.hasClassName(elem, b)) QZFL.css.updateClassName(elem, b, a)
            }
        },
        toggleClassName: function (elem, name) {
            ...
            if (_s.hasClassName(elem, name)) _s.updateClassName(elem, name);
    ...
    QZFL.dom = {
        ...
        removeElement: function (elem) {
            if (elem = QZFL.dom.get(elem)) {
                if (QZFL.userAgent.ie > 8 && elem.tagName ==
                    "SCRIPT") elem.src = "";
        ...
        getPreviousSibling: function (elem) {
            elem = QZFL.dom.get(elem);
            return elem.previousElementSibling || QZFL.dom.searchChain(elem && elem.previousSibling,
                    "previousSibling",
                    function (el) {
                        return el.nodeType == 1
                    })
        ...
        createElementIn: function (tagName, elem, insertFirst, attrs) {
            var _e = (elem = QZFL.dom.get(elem) || document.body).ownerDocument.createElement(tagName ||
                    "div"),
                k;
        ...
        insertAdjacent: function (elem, where, html, isText) {
            ...
            if (QZFL.lang.isElement(elem) && (pos[where] && (QZFL.lang.isString(html) || QZFL.lang.isElement(html)))) {
                if (elem.insertAdjacentHTML && (elem.insertAdjacentElement && elem.insertAdjacentText)) elem["insertAdjacent" + (typeof html == "object" ? "Element" : isText ? "Text" : "HTML")](pos[where],
                    html);
                else {
    ...
    QZFL.event = {
        ...
        purgeEvent: function (obj, type) {
            ...
            if (obj.eventsListUID && ((l = QZFL.event._eventListDictionary[obj.eventsListUID]) && (l[type] && (h = l[type].handlers))))
                for (var k in h)
                    if (obj.removeEventListener) obj.removeEventListener(type, h[k], false);
                    else if (obj.detachEvent) obj.detachEvent("on" + type, h[k]);
        ...
        getEvent: function (evt) {
            ...
            if (!evt) {
                ...
                while (c && ct < _s.MAX_LEVEL) {
                    if (c.arguments && ((evt = c.arguments[0]) && (typeof evt.button != "undefined" && typeof evt.ctrlKey != "undefined"))) break;
                    ++ct;
                    c = c.caller
                }
            }
        ...
        delegate: function (delegateDom, selector, eventType, fn, argsArray) {
            var eventData, handler, handlerObj, handlerArray, delegateType, res = false,
                E = QZFL.event;
            if (!delegateDom || (!selector || (!eventType || !fn))) return res;
            if (!delegateDom.eventsListUID) delegateDom.eventsListUID =
                "e" + ++E._objSeqUID;
            ...
            if (!eventData[delegateType]) eventData[delegateType] = {};
            handler = eventData.delegateHandler;
            if (!handler) handler = eventData.delegateHandler = function (e) {
                E.delegate._dispatch.apply(delegateDom, arguments)
            };
            handlerArray =
                eventData[delegateType].delegateHandlers;
            if (!handlerArray) {
                ...
                for (var i = 0, len = handlerArray.length; i < len; i++)
                    if (handlerArray[i].guid === fn.__elUID) return false;
                res = true
            }
            handlerObj = {
                "selector": selector,
        ...
        undelegate: function (delegateDom, selector, eventType, fn) {
            var eventData, handlerArray, delegateType, E = QZFL.event,
                len;
            ...
            if (!eventType) {
                for (var j in eventData) arguments.callee(delegateDom,
                    selector, j, fn);
                return
            }
            ...
            if (fn || selector)
                for (var i = 0, h; i < handlerArray.length;) {
                    h = handlerArray[i];
                    if (typeof fn == "function" && h.guid === fn.__elUID || selector === h.selector) handlerArray.splice(i, 1);
                    else i++
                } else handlerArray.length = 0;
            if (handlerArray.length === 0 && len !== 0) {
                delegateType = (E.delegate._specialEvent[eventType] || {}).delegateType ||
                    eventType;
                if (delegateDom.removeEventListener) delegateDom.removeEventListener(delegateType, eventData.delegateHandler, false);
                else if (delegateDom.detachEvent) delegateDom.detachEvent("on" + delegateType, eventData.delegateHandler);
                handlerArray = null;
    ...
    QZFL.object.each({"mouseenter": "mouseover", "mouseleave": "mouseout"}, function (delegateType, origType) {
            QZFL.event.delegate._specialEvent[origType] = {
                ...
                "handler": function (event, handlerObj) {
                    ...
                    if (target !== relatedTarget && !QZFL.dom.isAncestor(target, relatedTarget)) handlerObj.handler.apply(this, [event].concat(handlerObj.argsArray))
                }
            }
        });
    QZFL.event.delegate._dispatch = function (event) {
        var E = QZFL.event,
            ...
            type = event.type,
            eventData, handlerArray, hit, curDom, specialHandler,
            selector;
        ...
        for (curDom = E.getTarget(event); curDom != this; curDom = curDom.parentNode || this)
            for (var i = 0, len = handlerArray.length, handlerObj; i < len; i++) {
                ...
                if (hit) {
                    ...
                    else handlerObj.handler.apply(curDom, [event].concat(handlerObj.argsArray))
                }
            }
    };
    QZFL.queue = function () {
    ...
    QZFL.util = {
        ...
        URI: function (s) {
            ...
            if (s.indexOf("://") < 1) s = location.protocol + "//" + location.host +
                (s.indexOf("/") == 0 ? "" : location.pathname.substr(0, location.pathname.lastIndexOf("/") + 1)) + s;
            var depart = s.split("://");
    ...
    QZFL.lang = {
        ...
        isNode: function (o) {
            return o && (typeof o.nodeName !=
                "undefined" || typeof o.nodeType != "undefined")
        },
    ...
    (function () {
        ...
        QZFL.object.extend(QZFL.util, {
            copyToClip: function (text) {
                if (QZFL.userAgent.ie) return clipboardData.setData("Text", text);
                else {
    ...
    QZFL.lang.objectClone = function (obj, preventName) {
        if (typeof obj == "object") {
            ...
            for (var i in obj)
                if (i !=
                    preventName) res[i] = QZFL.lang.objectClone(obj[i], preventName);
            return res
    ...
    QZFL.lang.obj2str = function (obj) {
        ...
        if (typeof obj == "object") {
            ...
            "number" || typeof obj == "function") return obj.toString();
    ...
    QZFL.lang.propertieCopy = function (s, b, propertiSet, notOverWrite) {
        ...
        s = s || {};
        for (var p in l)
            if (!notOverWrite || !(p in s)) s[p] = l[p];
        return s
    };
    ...
    QZFL.lang.chain = function (u, v) {
        ...
        return function () {
            for (var i =
                0, len = calls.length; i < len; ++i)
                if (calls[i] && calls[i].apply(null, arguments) === false) return false;
            return true
        }
    };
    ...
    QZFL.pageEvents = function () {
        ...
        function _bootStrap() {
            ...
            window.onload =
                QZFL.lang.chain(window.onload, function () {
                    _onloadHook();
    ...
    (function () {
        ...
        var Sizzle = function (selector, context, results, seed) {
            ...
            if (!contextXML) {
                ...
                if (speedUpMatch) {
                    if (context.nodeType === 1 || context.nodeType === 9)
                        if (speedUpMatch[1]) return makeArray(context.getElementsByTagName(selector), results);
                        else if (speedUpMatch[2] || speedUpMatch[4] && speedUpMatch[5])
                            if (context.getElementsByClassName &&
                                speedUpMatch[2]) return makeArray(context.getElementsByClassName(speedUpMatch[2]), results);
                            else {
                    ...
                    if (context.nodeType === 9)
                        if ((selector === "body" || selector.toLowerCase() === "body") && context.body) return makeArray([context.body],
                            results);
                        else if (speedUpMatch[3]) return (tmpVar = context.getElementById(speedUpMatch[3])) ? makeArray([tmpVar], results) : makeArray([], results)
                }
            }
            do {
                chunker.exec("");
            ...
            if (parts.length > 1 && origPOS.exec(selector))
                if (parts.length === 2 && Expr.relative[parts[0]]) set = posProcess(parts[0] + parts[1], context);
                    ...
                    while (parts.length) {
                        ...
                        if (Expr.relative[selector]) selector +=
                            parts.shift();
            ...
            if (toString.call(checkSet) === "[object Array]")
                if (!prune) results.push.apply(results, checkSet);
        ...
        Sizzle.filter = function (expr, set, inplace, not) {
            ...
            while (expr && set.length) {
                ...
                if (expr === old)
                    if (anyFound == null) Sizzle.error(expr);
                    else break;
                old = expr
            }
            return curLoop
        ...
        var Expr = Sizzle.selectors = {
                ...
                NAME: function (match, context) {
                    if (typeof context.getElementsByName !== "undefined") {
                        ...
                        for (var i = 0, l = results.length; i < l; i++)
                            if (results[i].getAttribute("name") === match[1]) ret.push(results[i]);
            ...
            preFilter: {
                CLASS: function (match,
                                 curLoop, inplace, result, not, isXML) {
            ...
            filters: {
                ...
                button: function (elem) {
                    return "button" ===
                        elem.type || elem.nodeName.toLowerCase() === "button"
                },
                input: function (elem) {
            ...
            filter: {
                ...
                ATTR: function (elem, match) {
                    var name = match[1],
                        result = Expr.attrHandle[name] ? Expr.attrHandle[name](elem) : elem[name] != null ?
        ...
        try {
            ...
            makeArray = function (array, results) {
                ...
                if (toString.call(array) === "[object Array]") Array.prototype.push.apply(ret, array);
                    ...
                    for (var l = array.length; i < l; i++) ret.push(array[i]);
                else
            ...
            if (div.getElementsByTagName("*").length > 0) Expr.find.TAG = function (match, context) {
                ...
                if (match[1] === "*") {
                    ...
                    for (var i = 0; results[i]; i++)
                        if (results[i].nodeType === 1) tmp.push(results[i]);
                    results = tmp
                }
                return results
            };
        ...
        (function () {
            ...
            if (matches) Sizzle.matchesSelector = function (node, expr) {
                ...
                if (!Sizzle.isXML(node)) try {
                    if (pseudoWorks ||
                        !Expr.match.PSEUDO.test(expr) && !/!=/.test(expr)) return matches.call(node, expr)
                } catch (e) {
                }
                return Sizzle(expr, null, null, [node]).length > 0
            }
        ...
        var posProcess = function (selector, context) {
            ...
            for (var i =
                0, l = root.length; i < l; i++) Sizzle(selector, root[i], tmpSet);
    ...
    (function () {
        ...
        _handler.prototype = {
            _init: function (selector, context) {
                if (QZFL.lang.isString(selector)) this.elements =
                    QZFL.selector(selector, context);
            ...
            findElements: function (selector) {
                ...
                this.each(function (el) {
                    ...
                    if (_s.length > 0) _pushstack = _pushstack.concat(_s)
                });
                return _pushstack
            },
    ...
    QZFL.element.extendFn({
        ...
        setVal: function (value) {
            if (QZFL.object.getType(value) == "array") {
                ...
                this.each(function (el) {
                    if (/radio|checkbox/.test(el.type)) el.checked = el.nodeType && ("\x00" + _v.indexOf(el.value.toString() +
                            "\x00") > -1 || _v.indexOf("\x00" + el.name.toString() + "\x00") > -1);
        ...
        getVal: function (index) {
            ...
            if (_e)
                if (_e.tagName == "SELECT") {
                    ...
                    else QZFL.object.each(_e.options,
                        function (e) {
    ...
    QZFL.element.extendFn({
        ...
        getChildren: function () {
            ...
            this.each(function (el) {
                ...
                while (node) {
                    if (!!node && node.nodeType == 1) _arr.push(node);
    ...
    QZFL.element.extendFn({
        ...
        insertAfter: function (el) {
            var el = el.elements && el.elements[0] ||
                    QZFL.dom.get(el),
                _ns = el.nextSibling,
                _p = el.parentNode;
            return this.each(function (element) {
    ...
    QZFL.effect = {
        ...
        run: function (elem, prop, opts) {
            ...
            } else if (o.mode[1] == "css3" && opts.change == QZFL.emptyFn)(new QZFL.cssTransfrom(elem, fprop, opts)).firecss();
            else {
        ...
        show: function (elem, opts, cb) {
            ...
            QZFL.effect.run(elem, {opacity: 1}, QZFL.object.extend(opts, {
        ...
        hide: function (elem, opts, cb) {
            ...
            QZFL.effect.run(elem, {opacity: 0}, QZFL.object.extend(opts, {
                duration: duration ||
                1E3,
                complete: function () {
        ...
        slideToggle: function (elem, opts) {
            var o = QZFL.effect,
                status, opts = opts || {};
            if (o._isHidden(elem)) o.stop(elem).slideDown(elem,
                opts);
    ...
    QZFL.Deferred = function (func, args) {
        var _slice = Array.prototype.slice,
            ...
            Event = {
                ...
                _init: function (type) {
                    if (!this.eventList) this.eventList = {rejectFuncs: [], resolveFuncs: []}
                },
    ...
    var _autoXSS = function () {
        ...
        var _XSS = {
            go: function (item) {
                ...
                if (itemType === "string") rs = this.filter(item);
                    ...
                    if (itemObjType === "[object Array]" || itemObjType === "[object Arguments]") rs = this.parseArray(item);
                    else if (itemObjType === "[object Object]") rs = this.parseObject(item)
                }
                return rs
            },
        ...
        return function (data) {
            if (location.pathname.indexOf("/qzone/biz/") == -1) return data;
            else return _XSS.go(data)
        }
    }();
    (function () {
        var def = {
        ...
        function transdata(data) {
            ...
            if (typeof data == "object")
                for (var i in data) ar.push(i + "=" + encodeURIComponent(data[i]));
            else ar.push(data);
            return ar.join("&")
        }
        ...
        function isSameDomain(url) {
            ...
            var rs = /^https?\:\/\/([\s\S]+?)\//.test(url) && RegExp.$1 == cdm;
            return rs
        }
        ...
        function ajaxRequest(url, data, opt) {
            ...
            if (isSameDomain(url)) {
                ...
                xhr.onload = function () {
                    instance.endTime = +new Date;
                    var o = eval("(" + processText(this.responseText) + ")");
                    var cloneO = extend({}, o);
                    try {
                        o = _autoXSS(o)
                    } catch (e) {
        ...
        QZFL.ajaxGet = function (url, data, opt) {
            ...
            opt = extend({}, def, typeof opt == "function" ? {onload: opt} :
            opt || {});
    ...
    QZFL.imports.execFnQueue = function (arFn, isSuccess) {
        ...
        while (arFn.length) {
            ...
            if (QZFL.lang.isFunction(f)) setTimeout(function (fn) {
                return fn
            }(f), 0)
        }
    };
    ...
    QZFL.imports.on = function (type, fn) {
        if (!QZFL.imports.onQueue[type]) QZFL.imports.onQueue[type] = [];
    ...
    QZFL.imports.doOn = function (type, obj) {
        ...
        for (var i = 0, len = a.length; i < len; i++)
            if (typeof a[i] == "function") a[i](obj)
    };
    QZFL.FormSender = function (actionURL, method, data, charset) {
        ...
        if (typeof actionURL == "object" && (actionURL.nodeType == 1 && actionURL.tagName == "FORM")) this.instanceForm =
            actionURL;
        else {
    ...
    QZFL.FormSender._pluginsRunner = function (pType, data) {
        ...
        if (l && (len = l.length))
            for (var i = 0; i < len; ++i)
                if (typeof l[i] == "function") t = l[i](t) || data;
        return t
    };
    QZFL.FormSender._clear = function (o) {
        o._sender = o._sender.callback = o._sender.errorCallback = null;
        if (QZFL.userAgent.safari || QZFL.userAgent.opera) setTimeout('QZFL.dom.removeElement(document.getElementById("_fp_frm_' +
            o.name + '"))', 50);
    ...
    QZFL.FormSender.prototype.send = function () {
        ...
        return function () {
            ...
            var isspeedup = cookieflag && /(?:^user\.(s\d\.)?|\d{5,}\.|rc\.)qzone\.qq\.com/.test(host),
                realurl = this.uri + (this.uri.indexOf("?") > -1 ? "&" : "?") + "g_tk=" + QZFL.pluginsDefine.getACSRFToken(this.uri),
                ajaxurl = isspeedup ? "http://" + host + "/" : realurl,
                path, tmplURI,
                canUseXHR = win.QZFL.config.canUseXHR2;
            if (this.charset == "utf-8")
                if (1 && (win.QZFL && (win.QZFL.FormSender && (isspeedup && (win.QZFL.config.xhrProxyEnable && win.QZFL.config.xhrProxyEnable(realurl) == 1) || canUseXHR && canUseXHR(ajaxurl))))) {
                    ...
                    if (tmplURI.host.indexOf(".qzone.qq.com") != -1) path = "p/" + tmplURI.host.replace(".qzone.qq.com", "") +
                        tmplURI.pathname + tmplURI.search;
                    else path = "q/" + tmplURI.host.replace(".qq.com", "") + tmplURI.pathname + tmplURI.search;
                    this.speedup = 1;
                    return this.type = "xhr", win.QZFL.ajaxPost(isspeedup ? ajaxurl + path : ajaxurl, data, {
                        onload: this.onSuccess,
                        instance: this,
                        headers: isspeedup ? {"X-Real-Url": realurl} : {}
            ...
            if (!this.instanceForm) {
                ...
                if (ie || ff) ifrurl =
                    'javascript:document.open();document.domain="' + document.domain + '";var me=parent.QZFL.FormSender.instance["' + t.name + '"];document.write(me.ifrHTML);document.close();';
                ...
                ifrHTML = ifrHTML + '<form action="' + t.uri + (t.uri.indexOf("?") > -1 ? "&" : "?") + "g_tk=" + QZFL.pluginsDefine.getACSRFToken(t.uri) +
                    (window.g_qzonetoken ? "&qzonetoken=" + window.g_qzonetoken : "") + '" accept-charset="' + t.charset + '" id="p" enctype="application/x-www-form-urlencoded;charset=' + t.charset + '" method="post">';
    ...
    (function (callback) {
        ...
        while (_pool.length)
            if (callback(_cur = _pool.pop()) === true) return _cur;
        return null
    })
    (function (w) {
        if (w.QZFL && (w.QZFL.FormSender && w.QZFL.cookie)) {
    ...
    QZFL.JSONGetter._runFnQueue = function (q, resultArgs, th) {
        ...
        if (q && q.length)
            while (q.length > 0) {
                f = q.shift();
                if (typeof f == "function") f.apply(th ?
                    th : null, resultArgs)
            }
    ...
    QZFL.JSONGetter._pluginsRunner = function (pType, data) {
        ...
        if (l && (len = l.length))
            for (var i =
                0; i < len; ++i)
                if (typeof l[i] == "function") re = l[i](t);
        return re
    };
    QZFL.JSONGetter.prototype.send = function (callbackFnName) {
        ...
        var isspeedup = cookieflag && /(?:^user\.(s\d\.)?|\d{5,}\.|rc\.)qzone\.qq\.com/.test(host),
            realurl = da + function (oriurl) {
                    if (!window.g_qzonetoken) return "";
                    if (oriurl.indexOf("?") > -1) return "&qzonetoken=" + window.g_qzonetoken;
                    else return "?qzonetoken=" + window.g_qzonetoken
                }(da),
            tmplURI, path, ajaxurl = isspeedup ? "http://" + host + "/p" : realurl;
        if (isspeedup && QZFL.config.xhrGetReqProxyEnable(da)) {
            tmplURI = QZFL.util.URI(realurl);
            ...
            path =
                "/" + tmplURI.host.replace(".qzone.qq.com", "") + tmplURI.pathname + tmplURI.search;
            this.speedup = 1;
            return win.QZFL.ajaxGet(ajaxurl + path, null, {
                onload: function (th) {
                    return function () {
                        th._waiting = false;
                        th.endTime = +new Date;
        ...
        if (QZFL.userAgent.ie && ((typeof document.documentMode == "undefined" || document.documentMode <
            10) && !(QZFL.userAgent.beta && navigator.appVersion.indexOf("Trident/4.0") > -1))) {
    ...
    QZFL.media = {
        ...
        adjustImageSize: function (w,
                                   h, trueSrc, callback, errCallback) {
            var opts = {
                trueSrc: trueSrc,
        ...
        getFlashHtml: function (flashArguments, requiredVersion, flashPlayerCID) {
            ...
            if (QZFL.userAgent.ie)
                if (QZFL.userAgent.ie > 10) _attrs.push(' classid="clsid:', flashPlayerCID || "D27CDB6E-AE6D-11cf-96B8-444553540000", '"', ' data="', flashArguments.src ||
                    "", '"');
                else _attrs.push(' classid="clsid:', flashPlayerCID || "D27CDB6E-AE6D-11cf-96B8-444553540000", '"');
        ...
        insertFlash: function (containerElement,
                               flashArguments) {
            ...
            if (QZFL.userAgent.chrome && QZFL.userAgent.chrome >= 47) containerElement.innerHTML = QZFL.media.getFlashHtml(flashArguments);
            else {
    ...
    QZFL.media.reduceImage = function () {
        ...
        return function (type,
            ...
            if (opts.img)
                if (!(opts.direction && (opts.rate && (opts.longSize && opts.shortSize)))) r = QZFL.media.getImageInfo(function (o, p) {
                    if (!o || !p) return;
    ...
    QZFL.shareObject.getValidSO = function () {
        ...
        for (var i = 1; i < cnt + 1; ++i)
            if (QZFL.shareObject.instance["so_" + i] && QZFL.shareObject.instance["so_" + i]._ready) return QZFL.shareObject.instance["so_" + i];
        return null
    };
    ...
    QZFL.widget.msgbox = {
        ...
        show: function (msgHtml, type, timeout, opts) {
            ...
            if (typeof opts == "number") opts = {topPosition: opts};
            opts = opts || {};
    ...
    QZFL.dialog = {
        ...
        create: function (title, content, opts) {
            var t, args, dialog;
            if (content && (typeof content == "object" && (content.src && (content.src.indexOf("//") !== 0 && content.src.indexOf("/") === 0)))) content.src =
                window.location.protocol + "//" + (window.imgcacheDomain || "qzs.qq.com") + content.src;
            else if (content && typeof content == "string") content = content.replace(/<iframe([^>]+)>/ig, function () {
                var args = arguments;
                var attributeStr = args[1];
                return "<iframe " + attributeStr.replace(/src="([^"]+)"/i, function () {
                        var args = arguments;
                        var src = args[1];
                        if (src && (src.indexOf("//") !== 0 && src.indexOf("/") === 0)) src = window.location.protocol + "//" + (window.imgcacheDomain || "qzs.qq.com") + src;
                        return 'src="' + escHTML(src) + '"'
                    }) + "></iframe>"
            });
            if (t =
                    typeof opts != "number" || isNaN(parseInt(opts, 10))) {
                opts = opts || {};
    ...
    QZFL.widget.Confirm = function (title, content, opts) {
        if (typeof opts !=
            "undefined" && typeof opts != "object") opts = {type: opts, tips: arguments[3]};
        opts = opts || {};
    ...
    QZFL.maskLayout = function () {
        var masker = null,
            ...
            qml = function (zi, doc, opts) {
                ...
                masker.style.cssText = "background-color:#000;filter:alpha(opacity=" + 100 * opts.opacity +
                    ");opacity:" + opts.opacity + ";position:fixed;_position:absolute;left:" + opts.left + "px;top:" + opts.top + "px;z-index:" + zi + ";width:100%;height:" + QZFL.dom[QZFL.userAgent.ie < 7 ? "getScrollHeight" : "getClientHeight"](doc) + "px;";
                return count
            };
    ...
    QZFL.fixLayout = {
        ...
        create: function (html, isBottom, layerId, noFixed, options) {
            ...
            options =
                options || {};
            var tmp = {style: (isBottom ? "bottom:0;" : "top:0;") + (options.style || "left:0;width:100%;z-index:10000")},
                _c;
        ...
        fillHtml: function (layoutId, html) {
            this._layoutDiv[layoutId].innerHTML =
                html
        },
        _onscroll: function () {
        ...
        _doScroll: function () {
            ...
            for (var k in o._layoutDiv) {
                ...
                if (_item.isTop) o._layoutDiv[k].style.marginTop = QZFL.dom.getScrollTop() + "px";
                    ...
                    o._layoutDiv[k].style.marginBottom =
                        "0";
    ...
    QZFL.widget.tips = {
        ...
        show: function (html, aim, opts) {
            opts = opts || {};
            var bid =
                opts.id || "QZFL_bubbleTips_" + ++QZFL.widget.tips.count;
            opts.id = bid;
        ...
        resize: function (id) {
            QZFL.imports(QZFL.widget.tips.path, function () {
                if (QZFL.widget.tips) QZFL.widget.tips.resize &&
    ...
    QZFL.widget.seed = {
        ...
        update: function (k, opt) {
            ...
            if (typeof k == "undefined") n = th._update();
            else {
                k = th.prefix + k;
        ...
        get: function (k, opt) {
            ...
            if (typeof k == "undefined") return (th._seed = QZFL.cookie.get("randomSeed")) ? th._seed : th.update();
            else {
                k = th.prefix + k;
    ...
    QZFL.widget.runBox = function (startNode, endNode, opts) {
            ...
            QZFL.effect.run(dv, {left: ep.left, top: ep.top, width: ep.width, height: ep.height}, {
    ...
    QZFL.pluginsDefine.getACSRFToken = function (url) {
        ...
        var skey;
        if (url)
            if (url.host && url.host.indexOf("qzone.qq.com") > 0) try {
                skey = parent.QZFL.cookie.get("p_skey")
            } catch (err) {
                skey = QZFL.cookie.get("p_skey")
            } else if (url.host && url.host.indexOf("qq.com") >
                0) skey = QZFL.cookie.get("skey");
        ...
        if (t && t.pluginsPool) t.pluginsPool.formHandler.push(function (fm) {
            if (fm)
                if (!fm.g_tk) {
                    ...
                    a += (a.indexOf("?") > -1 ? "&" :
                            "?") + "g_tk=" + QZFL.pluginsDefine.getACSRFToken(a);
                    fm.action = a
                }
            return fm
        })
        ...
        if (t && t.pluginsPool) t.pluginsPool.srcStringHandler.push(function (ss) {
            ...
            if (typeof ss == "string")
                if (ss.indexOf("g_tk=") < 0) {
                    ...
                    if (jsRE.lastIndex = 0, !jsRE.test(pn)) ss += (sw ? "&" : "?") + "g_tk=" + QZFL.pluginsDefine.getACSRFToken(ss)
                }
            return ss
        })
    })();
    ...
    (function (q) {
        ...
        function valueStat(domain, cgi, type, code, time, rate, uin, exts) {
            ...
            var param = [];
            param.push("uin=" + uin, "key=" + "domain,cgi,type,code,time,rate", "r=" +
                Math.random());
            ...
            if (q[n] && q[n].pluginsPool) {
                ...
                q[n].pluginsPool.onRequestComplete.push(function (th) {
                    ...
                    if (d && (d = d[0])) {
                        ...
                        if (d.code != 0) collector.push([domain, url, 3, d.subcode || 1, +th.endTime - th.startTime, 1, th.speedup]);
                        else {
                            if (th instanceof q.JSONGetter)
                                if (th.reportRate)(th.reportRate == 1 || Math.random() < 1 / th.reportRate) && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || rateNumG, th.speedup]);
                                else isreportG &&
                                collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || rateNumG, th.speedup]);
                            if (th && (th.ownerWindow && th instanceof th.ownerWindow.QZFL.FormSender))
                                if (th.reportRate)(th.reportRate == 1 || Math.random() < 1 / th.reportRate) && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || rateNumP]);
                ...
                q[n].pluginsPool.onRequestComplete.push(function (th) {
                    if (isspeedup &&
                        (th && (th.ownerWindow && th instanceof th.ownerWindow.QZFL.FormSender))) {
                        var takes = th.postTime - th.startTime,
                            s, fl3;
                        if ((th._uri || th.uri).indexOf("taotao.qq.com") > -1) fl3 = 51;
                        else if ((th._uri || th.uri).indexOf("w.qzone.qq.com") > -1) fl3 = 52;
                        if (!fl3) return;
    ...
    try {
        if (!window.frameElement) QZFL.event.addEvent(window, "beforeunload", function () {
    ...
    QZFL.config = QZFL.config || {};
    var list = [
        ["taotao.qq.com", "emotion_cgi_publish_v6"],
        ["taotao.qq.com", "emotion_cgi_re_feeds"],
        ["taotao.qq.com", "emotion_cgi_addcomment_ugc"],
        ["taotao.qq.com", "emotion_cgi_addreply_ugc"], ""
    ].slice(0, -1);
    var re = /^https?\:\/\/([\s\S]+?)\//,
        rst;
    ...
    QZFL.config.xhrProxyEnable = function (url) {
        ...
        if (location.href.indexOf("proxyoff") > -1) return false;
        return url.indexOf("taotao.qq.com") > -1
    };
    QZFL.config.xhrGetReqProxyEnable = function (url) {
        if (QZFL.win.location.href.indexOf("proxyoff") > -1) return false;
        if (url.indexOf("//") == 0 || url.indexOf(".js") != -1) return false;
        ...
        var mtches = url.match(urlParse),
            dm = mtches[1],
            cgi = mtches[2];
        for (var i = 0; i < dms.length; i++)
            if (dms[i][0] == dm) {
                if (dms[i][2] == 0) return false;
                if (dms[i][1] == "*") return true;
                if (dms[i][1] == cgi) return true
            }
        return false
    };
    QZFL.config.canUseXHR2 = function (url) {
        ...
        for (var i = 0; i < list.length; i++)
            if (list[i][0] == rst[1] && (new RegExp("\\/" + list[i][1] + "\\b")).test(url)) return true;
        return false
    }
})();
